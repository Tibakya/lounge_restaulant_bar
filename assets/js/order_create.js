...
// const tableSelect = document.getElementById("table"); // Removed
// const waiterSelect = document.getElementById("waiter"); // Removed
let editOrderModal, modalBillNo, modalOrderIdInput, modalTableSelect,
    ...
    selectedTableIdHidden, selectedWaiterIdHidden; // Added for new selection

    // Define base_url globally
var base_url = '/RestaurantMS_CI/'; // Fallback
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Assign new hidden inputs for table/waiter ID
    ...
    if (typeof initialOrderItems === 'undefined' || Object.keys(initialOrderItems).length === 0) {
        if(orderBody) renderOrder();
    }

    // Event listeners for new table and waiter selection buttons
    ...
    if (tableButtonsContainer) {
        tableButtonsContainer.addEventListener('click', function(event) {
            if (event.target.classList.contains('table-button')) {
                ...
                if (selectedTableIdHidden) selectedTableIdHidden.value = event.target.dataset.tableId;
            }
        });
    }
    ...
    if (waiterButtonsContainer) {
        waiterButtonsContainer.addEventListener('click', function(event) {
            if (event.target.classList.contains('waiter-button')) {
                waiterButtonsContainer.querySelectorAll('.waiter-button').forEach(btn => btn.classList.remove('selected'));
                event.target.classList.add('selected');
                if (selectedWaiterIdHidden) selectedWaiterIdHidden.value = event.target.dataset.waiterId;
            }
        });
    }
});
...
function submitOrderForUpdate() {
    ...
    orderForm.submit(); // This will cause a page reload handled by the controller's redirect
}
...
function filterProductsClientSide(searchTerm = null, categoryId = null) {
    ...
    if (categoryId === null && categoryTabs) {
...
function openEditOrderModal(orderId) {
  if (!orderId || !editOrderModal) return;
...
function submitOrderUpdateFromModal(event) { // Added event parameter
    const modalForm = document.getElementById('edit-order-form-modal');
    if (!modalForm) {
